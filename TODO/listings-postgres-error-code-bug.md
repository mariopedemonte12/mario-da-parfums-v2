# Pending: real Postgres errors always report "Unexpected error"
...
An **implementation session** against this same worktree (or a follow-up
one) — not a testing session. Recommend fixing all three modules
(`listings`, `vendors`, `fragrances`) together in one pass, since it's the
same root cause and the same fix shape in each.
