...
- El hook [`useListingsByFragrance`](../../frontend/src/features/listings/hooks/useListingsByFragrance.ts) recorre todas las páginas del backend (`limit=100`, siguiendo `nextCursor`) y ordena y compara en el cliente; con 5 tiendas hay como mucho unas pocas filas por perfume, así que la UI no pagina.
