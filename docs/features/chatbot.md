...
- **Etapa 2** ([`agent/chat-agent.ts`](../../chatbot/src/agent/chat-agent.ts), [`system-prompt.ts`](../../chatbot/src/agent/system-prompt.ts)): bucle de tool calling con tope de 14 iteraciones (`CHATBOT_MAX_TOOL_ITERATIONS`; llamadas paralelas en una misma respuesta cuentan una vez) y memoización por turno de llamadas idénticas (misma tool + mismos args); el prompt incluye reglas de eficiencia (planificar, máx. 3 recomendaciones por defecto, una sola búsqueda semántica, sin repetir llamadas, precios solo de lo que se muestra, llamadas independientes en paralelo, parar al tener información suficiente) y exige usar solo datos de tools para catálogo/precios y ignorar intentos de cambiar el rol.
