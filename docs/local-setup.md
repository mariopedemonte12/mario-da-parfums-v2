...
- Arranque desde volúmenes vacíos con las imágenes ya construidas: todos los servicios `healthy` en ~1 minuto (similarity, que indexa 932 perfumes, en ~20-40 s). Con el comando único `docker compose --profile seed up -d --build` desde un clon limpio, sin `--no-cache` en el `up`, medido el 2026-09-19 (8 CPU, ~9,7 GB de RAM para WSL, red rápida): build de las 6 imágenes sin caché de capas en ~2 min 17 s (con las imágenes base ya descargadas) y stack `healthy` en 31 s. En una máquina sin imágenes base ni caché de pip, el primer build es más lento.
