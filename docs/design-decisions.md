...
> Formato: Problema → Alternativas → Decisión → Razón → Trade-offs. Cada dato se extrajo del código; las rutas citadas son relativas a esta carpeta. El servicio de similitud se llama `similarityServer` (carpeta `similarityServer/`; ver [`architecture.md`](architecture.md)).
...
- **Semántica**: `similarityServer`, `GET /search` ([`similarityServer/app.py`](../similarityServer/app.py)).
...
**Búsqueda textual: evolución y estado final.**
- *Antes*: `name` era filtro *contains* case-insensitive (comodines escapados, [`sql-like.util.ts`](../backend/src/common/utils/sql-like.util.ts)) y `brand` exacto; `jean` **no** encuentra "Jean Paul Gaultier".
- *Decisión final (implementada y en master)*: `search` es **el único mecanismo textual** (se eliminan los filtros exactos `name` y `brand`). Parte el texto en tokens (máx. 5 / 100 caracteres) y exige que cada token aparezca (`ILIKE`) en `name` **o** `brand`, sin importar el orden; índice trigrama `pg_trgm` sobre ambos campos. `jean` → Jean Paul Gaultier; `chanel bleu` → Bleu de Chanel. Detalle en [features/busqueda-textual.md](features/busqueda-textual.md). El frontend (`/fragrances` y el resolvedor de la búsqueda semántica) y la tool MCP `search_fragrances` ya usan `search`.
...
| Cómo se guardan los vectores | Archivo `embeddings.npz` (arrays `names`, `descriptions`, `embeddings`) + archivo hermano `.hnsw` con el grafo. **No** en Postgres | `PerfumeSimilarityIndex.save/load`; ruta por defecto `similarityServer/embeddings.npz`, configurable con `EMBEDDINGS_PATH` |
| Cómo se construye el índice | Al arrancar el servidor (`lifespan`): carga el `.npz` si existe, compara contra `fragrances` y **solo codifica lo nuevo o cambiado** (en un solo lote); si hubo cambios, reconstruye el grafo completo y guarda | [`index_sync.py`](../similarityServer/index_sync.py), `PerfumeSimilarityIndex.sync` |
| Índice | HNSW de `hnswlib`, espacio `ip` (producto interno), `M=16`, `ef_construction=200`, `ef_search=50` | `similarity.py` |
| Cómo se obtiene el resultado | `knn_query` devuelve los `k` vecinos; puntuación = `1 - distancia` (= coseno, por la normalización). `GET /search?q=&top_k=` (`top_k` 1–50, por defecto 5) responde `{results:[{name, score}]}` ordenado por similitud descendente | [`app.py`](../similarityServer/app.py) |
| Consumo en frontend | El frontend pide `top_k=4`, resuelve cada `name` contra `GET /fragrances?search=…&limit=50` (coincidencia exacta en el cliente) y muestra `round(score*100)` como "afinidad" | [`useFragranceSearch.ts`](../frontend/src/features/search/hooks/useFragranceSearch.ts), [`fragrances.api.ts`](../frontend/src/features/fragrances/api/fragrances.api.ts) |

**Razón.**
- *Modelo local*: evita depender de una API de pago y de red en cada búsqueda; el costo es una instalación pesada (`torch`). Se eligió un modelo multilingüe pequeño porque las descripciones y las consultas están en español ([`similarityServer/CLAUDE.md`](../similarityServer/CLAUDE.md)).
- *Por qué HNSW* ([`NOTES.md`](../similarityServer/NOTES.md)): sin fase de entrenamiento (IVF necesita k-means previo, incómodo para un catálogo que crece), buen recall/latencia sin ajustes, es un grafo en memoria sin servicio adicional, y su tiempo de consulta crece aproximadamente de forma logarítmica. `hnswlib` frente a `faiss`: implementación de referencia, dependencia mucho más pequeña.
...
- El índice identifica perfumes solo por `name` (el corpus es `(name, description)`), por lo que dos perfumes con el mismo nombre y distinta marca son ambiguos en el resultado. > TODO(verificar): comportamiento exacto con nombres duplicados entre marcas en el dataset real (hay al menos "Theoreme" y "Pour Homme EDT" según [`NOTES.md`](../similarityServer/NOTES.md)).
...
| MCP similitud (similarityServer, `/mcp`, nombre `perfume-similarity-search`) | `search_similar_fragrances` | búsqueda semántica: `query`, `top_k` (1–50); devuelve `{name, score}` | [`mcp_server.py`](../similarityServer/mcp_server.py) |
...
- `chatbot/mcp-servers.json` está vacío en el repo (fuera de Docker el agente no tiene tools hasta configurarlo). En Docker, `chatbot/mcp-servers.docker.json` declara backend (`http://backend:3000/mcp`) y similarity (`http://similarity:8001/mcp`).
...
**Decisión.** Archivo `.npz` + `.hnsw` en disco, cargado completo en memoria ([`NOTES.md`](../similarityServer/NOTES.md), "Why the embeddings index is a flat `.npz` file, not pgvector").
...
**Trade-offs.** Solo funciona con un proceso; si dos procesos necesitaran el índice, habría que revisarlo (lo dice la propia nota). Si el archivo se pierde, se recalcula todo (codificar el catálogo completo). En contenedores el archivo debe persistirse: el compose lo guarda en el volumen `similarity_data` (montado en `/data`, `SIMILARITY_EMBEDDINGS_PATH=/data/embeddings.npz`), así que un reinicio no recodifica el catálogo, pero borrar el volumen sí.
...
**Decisión** (fuente: [`docker-compose.yml`](../docker-compose.yml), [`specs/docker-infra.md`](../specs/docker-infra.md), los `Dockerfile` de cada paquete).
- Un solo `docker-compose.yml` en la raíz con seis servicios de larga vida o one-shot (`postgres`, `migrate`, `backend`, `similarity`, `chatbot`, `frontend`) y dos perfiles opcionales: `seed` (`seed-catalog`, `seed-prices`) y `tools` (`download-dataset`).
- Contexto de build = raíz del repo, porque los servicios Node resuelven dependencias con el `pnpm-lock.yaml` raíz. Imágenes multi-stage (backend, frontend, chatbot, similarityServer); el paquete Python fija versiones con `constraints.txt` y usa torch CPU. Los contenedores de backend, chatbot, similarity y priceGenerator corren con usuario no root (`USER` en su Dockerfile).
- Orden y salud con `depends_on` (`service_healthy` / `service_completed_successfully`): las migraciones de Drizzle corren en un servicio one-shot idempotente (`migrate`) en cada `up`; `similarity` solo declara salud cuando terminó de sincronizar el índice (`start_period` de 300 s); el chatbot espera a backend y similarity porque los usa por MCP.
- El índice `embeddings.npz` / `.hnsw` vive en el volumen `similarity_data`, así un reinicio solo re-codifica lo nuevo o cambiado. `postgres_data` persiste la base.
- Importador y priceGenerator se ejecutan como jobs de perfil `seed` (no como servicios permanentes) y en orden: catálogo primero, luego índice, luego precios. El dataset de Kaggle no está en el repo: se descarga con el job `download-dataset` (perfil `tools`) o se copia a mano.
