...
from similarityServer.config import load_config
from similarityServer.dataset_source import DEFAULT_CSV_PATH
...
def test_required_database_url_is_read_from_environment(monkeypatch):
    ...
    monkeypatch.delenv("SIMILARITY_DATASET_CSV_PATH", raising=False)
    monkeypatch.delenv("SIMILARITY_LOG_LEVEL", raising=False)
...
def test_dataset_csv_path_uses_explicit_override(monkeypatch):
    monkeypatch.setenv("DATABASE_URL", "postgresql://u:p@host/db")
    monkeypatch.setenv("SIMILARITY_DATASET_CSV_PATH", "/custom/path.csv")
